...
class DefensePipeline:
    ...
    def generate_defense_rules(
        ...
        try:
            # Resolve attack type BEFORE logging, clustering, prompt building, and RAG.
            # The old code only detected when attack_type was falsy, so a string like
            # "unknown" was passed straight into RAG and produced queries such as
            # "Naxsi unknown rule syntax ...".
            ...
            print("\tWAF Type:", waf_type.value)
            print("\tAttack Type:", attack_type)
            print("\tRaw Attack Type:", raw_attack_type)
            print(f"\tExisting Rules: {len(existing_rules) if existing_rules else 0}")
            for rule in (existing_rules or []):
                print(f"\t\t{rule}")
            print("\tBypassed payloads:")
            ...
            # Re-resolve here as a guard in case upstream supplied "unknown" or null.
            attack_type = self._resolve_attack_type(
                attack_type=attack_type,
                bypassed_payloads=bypassed_payloads,
                default="Unknown",
            )
            print(f"[DEFENSE] attack_type_sent_to_rag={attack_type!r}")
    ...
    def _is_unknown_attack_type(self, value: Any) -> bool:
        """Return True for missing or placeholder attack-type values."""
        if value is None:
            return True
        s = str(value).strip().lower()
        return s in self.UNKNOWN_ATTACK_TYPES

    def _normalize_attack_type_for_rag(self, value: Any) -> Optional[str]:
        """
        Normalize project attack labels to the compact labels expected by LLMShield RAG.

        Examples:
            xss_reflected, xss_stored, cross-site-scripting -> XSS
            sqli, sql_injection, sql injection             -> SQLI
            path_traversal, lfi                             -> LFI
        """
        if self._is_unknown_attack_type(value):
            return None
    ...
    def _extract_attack_type_from_payload_metadata(self, payloads: list[Any]) -> Optional[str]:
        """
        Extract explicit attack_type metadata from payload objects or serialized payload strings.

        This is different from guessing attack type from attack tokens. In your logs, each
        SimulatedPayload string contains a real field like:
            "attack_type": "xss_reflected"
        The previous implementation ignored this field and only counted tokens.
        """
        def visit(obj: Any) -> Optional[str]:
            ...
            if isinstance(obj, dict):
                for key in (
                    ...
                    if key in obj:
                        candidate = self._normalize_attack_type_for_rag(obj.get(key))
                        if candidate:
                            return candidate
            ...
            if isinstance(obj, str):
                ...
                for pattern in patterns:
                    ...
                    if match:
                        candidate = self._normalize_attack_type_for_rag(match.group(1))
                        if candidate:
                            return candidate
    ...
    def _resolve_attack_type(
        ...
        bypassed_payloads: list[Any],
        default: str = "Unknown",
    ) -> str:
        """
        Resolve the attack type that must be sent into RAG.

        Priority:
        1. explicit attack_type argument if it is not unknown/null
        2. explicit attack_type metadata embedded in payload objects/strings
        3. lexical fallback from payload content
        4. default
        """
        candidate = self._normalize_attack_type_for_rag(attack_type)
        if candidate:
            return candidate

        candidate = self._extract_attack_type_from_payload_metadata(bypassed_payloads)
        if candidate:
            return candidate

        candidate = self._detect_attack_type(bypassed_payloads)
        if not self._is_unknown_attack_type(candidate):
            return candidate
...
- Unique ID""",
    ...
    def _detect_attack_type(self, payloads: list[str]) -> str:
        """
        Best-effort lexical fallback only.

        The primary source of truth is explicit attack_type metadata from the pipeline or
        from SimulatedPayload objects. This fallback is used only when metadata is absent.
        """
        metadata_type = self._extract_attack_type_from_payload_metadata(payloads)
        if metadata_type:
        ...
        xss_keywords = [
            "xss", "cross site", "cross-site", "script", "<script", "</script",
            "onerror", "onload", "onclick", "onmouseover", "onfocus", "on_event",
            "alert", "confirm", "prompt(", "document.cookie", "javascript:",
            "<img", "img", "<svg", "svg", "iframe", "srcdoc", "eval(",
            "fromcharcode", "body onload", "svg onevent", "event handler",
        ]
        sql_keywords = [
            "sqli", "sql injection", "union select", "union", "select",
            "insert", "update", "delete", "drop", "sleep(", "benchmark(",
            "information_schema", "or 1=1", "and 1=1", "' or", '" or',
            "--", "#", "/*", "xp_cmdshell",
        ]
