...
def run_defend_pipeline(
    ...
    payload_objects = list(payloads or [])
    bypassed_payloads = [
        str(payload.payload)
        for payload in payload_objects
        if getattr(payload, "is_bypassed", None) and getattr(payload, "payload", None)
    ]
    parsed_existing_rules = _parse_existing_rules(existing_rules)

    clusters = _1_clustering(
        bypassed_payloads=bypassed_payloads,
        attack_type=attack_type,
    )
    ...
    valid_rules, invalid_rules = _4_validate_rules_syntax(
        generated_rules=generated_rules,
    )
    retried_rules = _5_retry_invalid_rules(
        waf_name=waf_name,
        invalid_rules=invalid_rules,
    )
    final_rules = _6_refine_rules(
        waf_name=waf_name,
        valid_rules=valid_rules + retried_rules,
        existing_rules=parsed_existing_rules,
    )
    ...
    return {
        ...
        "rag_sources": rag_sources,
        "generated_rules": generated_rules,
        "advanced_defense": bool(parsed_existing_rules),
        "existing_rules_count": len(parsed_existing_rules),
        "final_rules": final_rules,
        "stats": {
            "total_payloads": len(context.get("payloads", [])),
            "num_clusters": len(context.get("clusters", [])),
            "rules_generated": len(context.get("generated_rules", [])),
            "rules_valid": len(context.get("valid_rules", [])),
            "rules_invalid": len(context.get("invalid_rules", [])),
            "rules_retried": len(context.get("retried_rules", [])),
            "rules_refined": len(context.get("final_rules", [])),
